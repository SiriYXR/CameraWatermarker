...
def get_image_meta_info(img_file):
...
def camera_watermarker(img_file,output_dir,output_suffix,border=0.03):
    ...
    meta_dic=get_image_meta_info(img_file)
    print(meta_dic)
    ...
    wm_heigh = int(org_img_h*0.1) # 原始照片高度的 10%
    ...
    device_model_x = 20
    device_model_y = 20
    device_model_w = len(meta_dic['device_model']) * font_size_1

    # 镜头型号
    lens_model_x = device_model_x
    lens_model_y = device_model_y+130
    lens_model_w = len(meta_dic['lens_model']) * font_size_2

    # 左侧水印信息绘制
    left_wm_img_width = 800
    left_wm_img_heigh = 250
    left_wm_img=Image.new(org_img.mode,(left_wm_img_width,left_wm_img_heigh),'white')
    ...
    # 右侧水印信息绘制
    right_wm_img_width = 2600
    right_wm_img_heigh = 250
    right_wm_img=Image.new(org_img.mode,(right_wm_img_width,right_wm_img_heigh),'white')
    right_wm_img_draw = ImageDraw.Draw(right_wm_img)

    # 绘制水印 logo
    img_sony_logo = Image.open('./src/sony_logo.jpg')
    img_sony_logo_w = img_sony_logo.size[0]
    img_sony_logo_h = img_sony_logo.size[1]
    img_sony_logo_x = 20
    img_sony_logo_y = int((right_wm_img_heigh-img_sony_logo.size[1])/2)
    right_wm_img.paste(img_sony_logo,(img_sony_logo_x,img_sony_logo_y))

    # 绘制分割线
    split_line_x= img_sony_logo_w+int(left_wm_img_heigh*0.2)
    split_line_y= 20
    split_line_w= int(right_wm_img_heigh*0.02)
    split_line_h= int(right_wm_img_heigh*0.8)
    ...
    # 拍摄参数
    img_exposure_value_x = split_line_x + int(left_wm_img_heigh*0.2)
    ...
    # 绘制水印到输出图片
    left_wm_img=left_wm_img.resize((int(left_wm_img_width*wm_heigh/left_wm_img_heigh),wm_heigh))
    right_wm_img=right_wm_img.resize((int(right_wm_img_width*wm_heigh/right_wm_img_heigh),wm_heigh))
    if left_wm_img.size[0] + right_wm_img.size[0] > wm_width :
        new_wm_img = Image.new(org_img.mode,(left_wm_img.size[0] + right_wm_img.size[0] + border_w,wm_heigh),'white')
        new_wm_img.paste(left_wm_img,(0,0))
        new_wm_img.paste(right_wm_img,(left_wm_img.size[0] + border_w,0))
        new_wm_img=new_wm_img.resize((wm_width,int(new_wm_img.size[1] * wm_width/new_wm_img.size[0])))
        res_img.paste(new_wm_img,(border_w, int(border_w + org_img_h + (wm_heigh+border_w-new_wm_img.size[1])/2)))
    else :
        res_img.paste(left_wm_img,(border_w,int(border_w + org_img_h + (wm_heigh+border_w-wm_heigh)/2)))
        res_img.paste(right_wm_img,(wm_width - border_w-right_wm_img.size[0],int(border_w + org_img_h + (wm_heigh+border_w-wm_heigh)/2)))
...
if __name__ == '__main__':  
    ...
    camera_watermarker(img_file,output_dir,output_suffix,0.03)